...
bot.on('message', message => {
  ...
  if (message.content === '~help') {
    message.channel.send("歡迎使用 煙花谷v.142 Discord簽到系統，本系統由嘎嘎獨立製作 \n若您還未建立帳戶，請輸入【~create】以建立您的帳號(請將帳號設定與煙花谷相同)\n　進行簽到請輸入 【~sign】\n查看簽到天數請輸入【~check】\n\n"+config.version);
  }
  if (message.content === '~check') {
			...
			db.query("SELECT date,day FROM discord WHERE dis_id='"+id+"'",[],function(err,rows){
				...
				if(obj[0] == null){
					...
					message.reply('您尚未建立帳號，輸入【~create】以建立您的帳號(請將帳號設定與煙花谷相同)');
					return false;
				}
				else{
  ...
  if (message.content === '~create') {
			...
			db.query("INSERT INTO discord VALUES(NULL,'"+id+"','0','"+now+"')",[],function(err,rows){
				...
				if(msg != 'null'){
					...
					message.reply('您已經建立帳號了，趕快下載遊戲吧！');
					return false;
				}
				console.log('[INFO] Success create at:'+id);
				message.reply('帳號建立成功!');
			});
  }
